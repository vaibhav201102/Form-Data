...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == requestGallery && data != null) {
                ...
                ImagePicker.getFile(data)?.let { doRequest(it,fileUri) } ?:

                run{ Toast.makeText(applicationContext,"Ops. Something went wrong.",Toast.LENGTH_SHORT)}
    ...
    private fun doRequest(file:File,uri: Uri) {
        ...
        serviceBuilder.uploadImage(692,body).enqueue(object : retrofit2.Callback<Result> {
